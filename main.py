...
STRINGS = {
    ...
    "en": {
        ...
        "invite_locked": "❌ <b>Locked!</b> You must buy a course first to get an invite link.",
    ...
    if call.data.startswith("setlang_"):
        ...
        if t_id in u_data and course:
            ...
            if l1 and l1 in u_data:
                u_data[l1]["balance"] += course.get("l1", 0)
...
def handle_menu(message):
    ...
    # PROFILE (FIXED JOIN DATE)
    if text in ["👤 प्रोफाइल", "👤 Profile"]:
        p = data[uid]
        if "join_date" not in p or not p["join_date"]:
            p["join_date"] = time.strftime("%Y-%m-%d")
            data[uid] = p
            save_json(DB_FILE, data)
        bot.send_message(uid, STRINGS[lang]["profile"].format(name=p['name'], status=p['status'], refs=p.get('referrals', 0), bal=p['balance'], date=p["join_date"]), parse_mode="HTML")

    # INVITE LINK (FIXED: ONLY FOR PAID USERS)
    elif text in ["🔗 इनवाइट लिंक", "🔗 Invite Link"]:
        if data[uid].get("status") == "Paid":
